...
func (s *SignerService) ListKeyInfo(ctx context.Context) error {
	for i := 0; i < len(s.eth.config.Auth); i++ {
		...
		s.eth.logger.Info("keyinfo:", "name", auth.ClientName, "keyId", auth.KeyName, "pubkey", hex.EncodeToString(pubkey), "addr", address)
	}
	return nil
}
